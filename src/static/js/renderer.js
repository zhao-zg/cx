...
(function (win) {
  ...
  function setContent(html, viewType, batchPath, chapter, training) {
    ...
    try { if(window.Capacitor||window.navigator.standalone||(window.matchMedia&&(window.matchMedia('(display-mode: standalone)').matches||window.matchMedia('(display-mode: fullscreen)').matches||window.matchMedia('(display-mode: minimal-ui)').matches))){sessionStorage.setItem('cx_access','ok');try{document.cookie='cx_access=ok;path=/;max-age=31536000;samesite=lax';}catch(e){};} } catch(e) {}
    setMeta(training);
