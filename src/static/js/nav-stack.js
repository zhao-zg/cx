(function() {
    ...
    function isPWA() {
        return (window.matchMedia && (window.matchMedia('(display-mode: standalone)').matches || window.matchMedia('(display-mode: fullscreen)').matches || window.matchMedia('(display-mode: minimal-ui)').matches)) || window.navigator.standalone === true;
    }
...
(function() {
    ...
    document.addEventListener('click', function(e) {
        ...
        /* 有浮动菜单/面板正开着时，本次点击属于操作弹框（关闭或内部交互），不弹浮动栏 */
        if (clickOutsideFloatingMenu(e)) return;
