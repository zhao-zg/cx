    ...
    function showGuideDialog() {
        ...
        var html = '<div class="cx-dialog" style="max-width:420px;padding:0;position:relative;max-height:80vh;display:flex;flex-direction:column">' +
            ...
            '<div style="flex:1;overflow-y:auto;padding:12px 16px 16px;line-height:1.6;font-size:13px;color:var(--text)">' +
                _guideSec('🎨 阅读设置', [
                    ['🌓', '主题切换', '暖色 / 冷色 / 夜间三种模式，未手动选择时跟随手机深浅色自动切换'],
                    ['🔤', '字体大小', '拖动滑块调节字号，设置自动保存，所有页面生效']
                ]) +
                _guideSec('📚 内容浏览', [
                    ['📋', '纲目', '大纲结构视图，顶部有「大纲 / 大中纲 / 全部」按钮控制展开层级，点击每级前的符号可单独展开或收起'],
                    ['📖', '听抄', '信息全文，多层级结构（大点、中点、小点等），适合深度阅读'],
                    ['🌅', '晨读翻页', '顶部星期标签可快速跳转，左右滑动或点箭头按钮切换，打开时自动定位到当天'],
                    ['🌄', '晨读内容', '每天含纲目、晨兴喂养、信息选读、参读等板块'],
                    ['🎵', '诗歌', '诗歌图片浏览，点击图片可放大查看'],
                    ['📝', '职事摘录', '职事信息摘录，按段落分段展示'],
                    ['📜', '标语', '训练标语浏览，从目录页顶部导航进入'],
                    ['🖼️', '标语诗歌', '标语诗歌图片浏览，从目录页或标语页进入'],
                    ['📑', '视图记忆', '每篇自动记住上次的视图（纲目 / 听抄 / 晨读等），下次打开直接进入']
                ]) +
                _guideSec('🔊 朗读功能', [
                    ['▶️', '播放控制', '底部控制栏：播放 / 暂停、进度条拖动、时间显示'],
                    ['⏩', '变速朗读', '支持 0.5x ~ 2x 六档语速，朗读中也可随时切换'],
                    ['🔄', '循环播放', '点击循环按钮，反复朗读当前页面'],
                    ['💡', '朗读高亮', '朗读时自动高亮当前句子，跟随进度移动，方便跟读'],
                    ['📖', '经文朗读', '朗读时经文缩写自动读为完整书名（如「太五3」读作「马太福音五章三节」）'],
                    ['📱', '后台朗读', '支持锁屏和后台朗读，通知栏显示标题和控制按钮'],
                    ['🎧', '锁屏控制', '锁屏界面显示篇章标题，可直接控制播放 / 暂停 / 停止']
                ]) +
                _guideSec('📜 经文系统', [
                    ['✝️', '经文弹框', '点击经文引用（如「太五3」），弹出窗口显示完整经文'],
                    ['🔗', '串珠经文', '弹框内点击串珠编号，展开相关经文，可继续点击查看'],
                    ['📌', '注脚查看', '弹框内点击注脚编号，展开对应注解内容'],
                    ['🔍', '自动识别', '正文中的中文经文引用（如「罗马书五章十七节」）自动识别为可点击链接'],
                    ['📖', '读经横幅', '每篇顶部「读经」栏可点击，查看所有引用经文']
                ]) +
                _guideSec('🔍 全文搜索', [
                    ['🔎', '搜索入口', '主页和各页面顶部的 🔍 按钮均可打开搜索'],
                    ['📝', '多词搜索', '输入多个词（空格分隔），匹配任意一个即显示'],
                    ['📂', '分组展示', '结果按训练分组，显示篇号、类型和预览'],
                    ['📍', '精准跳转', '点击结果直接跳转到对应段落，关键词高亮显示']
                ]) +
                _guideSec('✏️ 划线笔记', [
                    ['🖍️', '添加划线', '长按选择文字即可添加高亮、颜色标记或下划线'],
                    ['🗑️', '删除标记', '点击已有划线，在弹出菜单中点击「删除」移除'],
                    ['🎨', '修改标记', '点击已有划线，选择「修改 ▾」更改颜色或样式'],
                    ['📓', '添加笔记', '点击划线旁的 📝 图标，或选择「笔记」按钮输入内容'],
                    ['📝', '编辑笔记', '选择「修改笔记」编辑内容，或点击「删除」移除笔记'],
                ...
                _guideSec('⚙️ 其他功能', [
                    ['🧭', '浮动导航栏', '向下滚动后顶部出现快捷导航，含视图切换和设置按钮，5 秒无操作自动收起'],
