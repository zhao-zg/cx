...
- **新增** 赞助商配置（sponsor_enabled）
  ...
  - 默认值为 false，关闭状态下不会发布赞助二维码图片，前端自动隐藏赞助入口
...
#### 后端处理逻辑
- 图片发布控制：当 sponsor_enabled 为 false 时，跳过发布 zanzhu-wx.png 和 zanzhu-zfb.jpg 二维码图片
- 远程配置注入：在生成 remote-config.js 时，将 sponsorEnabled 参数传递给前端
...
```mermaid
flowchart TD
Config["config.yaml<br/>sponsor_enabled: false"] --> Load["main.py<br/>load_config()"]
...
- **新增** 赞助商条件发布：关闭赞助商功能可减少 2 个二维码图片文件的发布，节省存储空间
