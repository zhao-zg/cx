...
S->>S : 解析引用/构建段落映射
alt 使用 NativeTTS
S->>N : speak(text, lang, rate, startSecs, totalSecs, loop)
...
## 结论
CX 项目的前端以模块化与事件驱动为核心，结合路由、渲染器、主题设置、语音合成与导航栈，形成完整的 SPA 体验，并通过 PWA 与 Capacitor 能力实现跨平台稳定运行。通过合理的缓存策略与性能优化手段，兼顾了离线可用性与交互流畅性。建议持续关注引擎兼容性、缓存完整性与用户反馈渠道，以进一步提升可靠性与可维护性。