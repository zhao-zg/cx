...
## 更新摘要
**变更内容**
- 新增 _isPrebuiltCacheStale() 缓存验证功能，解决多页阅读界面中缓存过期问题
- 更新预合成状态跟踪机制，增强翻页后的缓存有效性检测
- 优化预合成文件管理和播放流程，提升多页阅读体验
- 改进事件驱动停止机制，确保缓存状态的准确同步
...
系统采用前后端分离的架构设计，后端使用Python处理文档解析和静态页面生成，前端使用JavaScript实现SPA（单页应用）界面和TTS功能。**更新** 系统现已集成重大改进，包括新增的缓存验证功能、增强的预合成状态跟踪机制、改进的错误处理机制、优化的播放流程和事件驱动停止机制，显著提升了TTS服务的可靠性、性能和用户体验。
...
### _isPrebuiltCacheStale() 缓存验证功能

**更新** 新增的缓存验证功能，专门解决多页阅读界面中的缓存过期问题：

#### 缓存验证机制
- **活动页面检测**: 通过 `document.querySelector('.day-page.is-active')` 获取当前活动页面
- **元素包含检查**: 验证预构建的_segmentMap中的元素是否包含在活动页面内
- **过期状态判断**: 如果缓存元素不在当前活动页内，则标记为过期

#### 多页阅读支持
- **晨读视图**: 支持多天分页（.day-page）的晨读视图
- **预构建时机**: `prebuildText()` 在页面加载时执行，但initDayPager尚未运行
- **活动页回退**: 当无活动页标记时，回退到第一个day-page（第一天）

#### 缓存清理策略
- **过期检测**: 当检测到缓存过期时，清除 `_prebuiltFullText` 和 `_prebuiltSegmentMap`
- **重新构建**: 触发重新预构建文本和segmentMap
- **状态同步**: 确保预构建缓存与当前显示内容保持一致

#### 预合成优化
- **翻页后处理**: 在预合成前检查缓存有效性
- **条件清理**: 仅在缓存过期时清理预构建缓存
- **性能优化**: 避免不必要的重新构建，提升预合成效率
...
subgraph "缓存验证层"
CC[_isPrebuiltCacheStale函数] --> DD[活动页面检测]
DD --> EE[元素包含检查]
EE --> FF[缓存过期判断]
FF --> GG[预构建缓存清理]
end
...
subgraph "预合成文件管理层"
LL[增强文件检查] --> MM[自动重合成机制]
MM --> NN[预生成流水线优化]
NN --> OO[损坏文件处理]
end
subgraph "播放流程优化层"
PP[MediaPlayer复用] --> QQ[播放参数优化]
QQ --> RR[播放完成处理]
RR --> SS[播放位置广播]
end
subgraph "日志记录增强层"
TT[emitLog方法] --> UU[标准Android日志]
UU --> VV[JS控制台转发]
VV --> WW[实时性能监控]
end
```
...
**图表来源**
- [src/static/js/speech.js:448-457](file://src/static/js/speech.js#L448-L457)
- [src/static/js/speech.js:1211-1218](file://src/static/js/speech.js#L1211-L1218)
- [src/static/js/speech.js:1384-1388](file://src/static/js/speech.js#L1384-L1388)
...
E --> F[等待ttsStopped事件]
F --> G{事件到达?}
G --> |是| H[收到事件后_waitStopDone=false]
G --> |否| I[5秒超时强制继续]
H --> J[发送preSynthesize]
I --> J
J --> K[音频播放]
...
**更新** 系统现已集成多项重大改进，包括新增的缓存验证功能、三层重试策略、增强的预合成文件管理、改进的错误处理机制、优化的播放流程和事件驱动停止机制，显著提升了TTS系统的性能、稳定性和用户体验：
...
**更新** 缓存验证功能、事件驱动停止机制、三层重试策略、预合成文件管理和MediaPlayer复用等改进带来的系统性能优化：
...
该系统适用于需要处理大量训练材料并提供高质量阅读体验的应用场景，新增的多项改进为开发者提供了更强大、更可靠的TTS服务支持，显著提升了系统的稳定性和用户体验。通过事件驱动停止机制、三层重试策略、增强的预合成文件管理、改进的播放流程、MediaPlayer复用策略和新增的缓存验证功能，系统现在能够在各种复杂的使用场景下提供更加稳定和可靠的TTS服务。