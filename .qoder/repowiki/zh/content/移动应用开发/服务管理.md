...
## 更新摘要
**所做变更**
- 新增TTS前台服务延迟合成逻辑优化细节：补充了50毫秒延迟如何解决页面切换时引擎静默丢弃问题
- 新增speakGen验证防止过期回调机制：说明了生成器验证如何确保并发speak时的幂等与一致性
- 新增预合成超时保底机制：补充了4秒超时检测如何避免合成被引擎静默丢弃
- 新增延迟合成优化策略：说明了预合成期间的延迟策略如何提升页面切换体验
...
JS->>SP : 页面切换/播放控制
SP->>SVC : startForegroundService(ACTION_SPEAK)
SVC->>NM : 创建通知通道/构建通知
SVC->>AM : 申请音频焦点/获取唤醒锁
...
### 延迟合成逻辑优化
- 预合成超时保底机制
  - 在 handleSpeak 中添加 4 秒超时检测，防止合成被引擎静默丢弃
  - 通过文件大小检查（<1KB 说明只有 WAV 头或为空）判断合成是否成功
  - 超时后自动重新合成，避免无限等待
- 50 毫秒延迟策略
  - 在 handlePreSpeak 中添加 50ms 延迟，解决页面切换时 tts.stop() 与 synthesizeToFile 的竞争问题
  - 通过 speakGen 检查防止过期回调，不依赖 isStopped 状态
  - 确保预合成文件在页面切换后仍能正确生成
- speakGen 验证机制
  - 通过 generation 检查防止过期回调影响当前播放状态
  - 在 onDone/onError 中验证 gen 是否匹配当前 speakGen
  - 预合成模式下 isStopped=true 但文件应保留，供后续 handleSpeak 复用
...
- 延迟合成问题
  - 预合成超时：检查 4秒超时检测逻辑，确认文件大小检查是否正确
...
**新增的重要改进包括：**
- **延迟合成逻辑优化**：通过 4 秒超时保底机制和 50 毫秒延迟策略，解决了页面切换时引擎静默丢弃问题
- **generation 验证机制**：通过 speakGen 检查防止过期回调影响当前播放状态，确保并发操作的幂等性
- **预合成超时保底**：handleSpeak 中的超时检测避免合成被引擎静默丢弃，提升稳定性
- **延迟合成优化**：handlePreSpeak 中的 50ms 延迟策略解决页面切换时的竞争问题
