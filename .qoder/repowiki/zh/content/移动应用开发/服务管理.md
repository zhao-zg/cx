...
## 更新摘要
**所做变更**
- 新增 onServiceStopped Runnable 回调机制：增强 stop 操作的可靠性和事件驱动同步
- 改进的预合成逻辑：优化 handlePreSpeak 中的引擎清理和合成策略
- 增强的事件驱动同步机制：通过 ttsStopped 事件实现 JS 与 Java 的精确同步
- 更可靠的停止操作处理：改进 handleStop 中的引擎清理和资源释放
- 优化的线程安全机制：确保所有 TTS 操作在 ttsHandler 线程上执行
- 改进的延迟合成处理：增强对静默丢弃或阻塞的检测和恢复能力
...
### 重大架构改进
**更新** 基于 Applied Changes: 重大架构改进，新增 onServiceStopped Runnable 回调机制、改进的预合成逻辑、增强的事件驱动同步机制

- **新增 onServiceStopped Runnable 回调机制**
  - 在 NativeTTSPlugin.stop() 中设置 onServiceStopped 回调，确保 stop 操作完成后通知 JS
  - handleStop() 完成引擎清理后，通过 mainHandler.postDelayed(r, 100) 通知 Plugin
  - JS 通过 ttsStopped 事件监听，收到后再安全启动 preSynthesize，替代硬编码 3s 延迟
  - 提供更精确的事件驱动同步，避免竞态条件和不必要的等待

- **改进的预合成逻辑**
  - handlePreSpeak 中的引擎清理策略：在 ttsHandler 线程上执行 tts.stop()，确保与合成在同一线程
  - 200ms 延迟策略：确保上一个页面的 handleStop 在 ttsHandler 上投递的 tts.stop() 完全执行
  - 预合成模式下 isStopped=true 但允许合成，通过 isPreSynthesis 标志区分
  - 改进的合成阻塞处理：通过 synthForChunk 状态检查和 generation 验证，增强对静默丢弃的检测

- **增强的事件驱动同步机制**
  - speech.js 中的 _waitStopDone 标记：stop → ttsStopped 事件驱动等待标记
  - 替代之前的盲等 3s 定时器方案，通过事件驱动实现精确同步
  - _waitStopGen 生成器：确保跨页面操作的幂等性
  - ttsStopped 监听器：在收到事件后清除标记并发送 preSynthesize

- **更可靠的停止操作处理**
  - handleStop() 中的 50ms 延迟：给引擎一个完成当前合成的窗口，避免中途强制截断
  - 改进的 speakGen 管理：在 ttsHandler 线程上投递 tts.stop()，避免主线程调用导致的异常状态
  - 优化的资源释放：不立即销毁 Service，保留 TTS 引擎实例，下次播放无需重新初始化
  - 增强的清理策略：不调 cleanTempFiles()，避免删除正在写的文件导致引擎异常

- **优化的线程安全机制**
  - 所有 TTS 相关操作（tts.stop()、synthesizeToFile、speak）都在 ttsHandler 线程执行
  - mainHandler 与 ttsHandler 分工明确，避免阻塞
  - 改进的 postDelayed 调用链：确保操作顺序和时序正确
  - 增强的 generation 验证：防止过期回调影响当前播放状态

**章节来源**
- [TTSForegroundService.java:78-84](file://android/app/src/main/java/com/tehui/offline/TTSForegroundService.java#L78-L84)
- [TTSForegroundService.java:849-905](file://android/app/src/main/java/com/tehui/offline/TTSForegroundService.java#L849-L905)
- [TTSForegroundService.java:747-822](file://android/app/src/main/java/com/tehui/offline/TTSForegroundService.java#L747-L822)
- [NativeTTSPlugin.java:129-133](file://android/app/src/main/java/com/tehui/offline/NativeTTSPlugin.java#L129-L133)
- [speech.js:173-179](file://src/static/js/speech.js#L173-L179)
...
- **线程安全优化**
  - 所有TTS操作在ttsHandler线程执行，避免主线程调用导致的静默丢弃
  - 4秒超时保底机制：防止合成被引擎静默丢弃
  - 50毫秒延迟策略：解决页面切换时的竞争问题
  - generation 验证：确保并发操作的幂等性
- **cleanup逻辑优化**
  ...
  - 页面切换清理改进：通过80ms延迟确保引擎完全重置，解决tts.stop()与synthesizeToFile的竞争问题
- **新增的事件驱动同步机制**
  - onServiceStopped Runnable 回调：提供精确的停止操作完成通知
  - ttsStopped 事件驱动：替代硬编码延迟，实现更可靠的同步
  - 改进的预合成逻辑：通过引擎清理和合成策略优化，提升预合成成功率
...
- **线程安全问题**
  - 现象：主线程调用tts.stop()导致的静默丢弃
  - 处理：确认所有TTS操作都在ttsHandler线程执行，检查postDelayed调用链
  - 超时合成问题：检查4秒超时检测逻辑，确认使用mainHandler而非ttsHandler
  - generation冲突：检查generation验证机制是否正常工作
- 延迟合成问题
...
- **新增的事件驱动同步问题**
  - onServiceStopped 回调：检查 NativeTTSPlugin 中的回调设置和调用
  - ttsStopped 事件：验证事件监听器是否正确注册和移除
  - 预合成等待：确认 _waitStopDone 标记和 _waitStopGen 生成器的正确使用
  - 停止操作可靠性：检查 handleStop 中的引擎清理和资源释放逻辑
...
## 结论
本服务管理方案通过前台服务、媒体会话、音频焦点与唤醒锁组合，确保 TTS 在后台稳定运行；通过分片与预生成策略、精确的位置与进度上报，提供流畅的用户体验；通过插件桥接与启动画面联动，实现 JS 与原生层的高效协作。最新的重大架构改进通过新增 onServiceStopped Runnable 回调机制、改进的预合成逻辑、增强的事件驱动同步机制，显著提升了系统的稳定性和可靠性。这些改进包括：

- **重大线程安全改进**：修复了主线程调用tts.stop()导致的静默丢弃问题
- **新增事件驱动同步机制**：通过 onServiceStopped 回调和 ttsStopped 事件实现精确同步
- **改进的预合成逻辑**：优化引擎清理和合成策略，提升预合成成功率
- **增强的停止操作处理**：改进 handleStop 中的引擎清理和资源释放
- **优化的延迟合成处理**：增强对静默丢弃或阻塞的检测和恢复能力
- **改进的 generation 验证机制**：确保并发操作的幂等性
