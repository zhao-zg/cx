...
## 更新摘要
**所做变更**
- 更新超时机制优化：将4秒超时检测从ttsHandler改为mainHandler，避免线程阻塞问题
- 改进页面切换引擎清理：在handlePreSpeak中增加80ms延迟，确保引擎完全重置
- 增强合成阻塞处理：优化对合成被引擎静默丢弃或阻塞的检测和恢复机制
...
### 超时机制优化与页面切换引擎清理
- **超时检测从ttsHandler改为mainHandler**
  - 在 handleSpeak 中的 4 秒超时检测现在使用 mainHandler.postDelayed 而非 ttsHandler.postDelayed
  - 避免了合成阻塞 ttsHandler 线程导致超时检测无法执行的问题
  - 确保即使合成过程阻塞，也能及时触发超时恢复机制
- **页面切换时的引擎清理改进**
  - 在 handlePreSpeak 中增加 80ms 延迟，确保引擎完全重置
  - 与 handleSpeak 正常路径保持一致的清理模式
  - 通过 ttsHandler 线程上的重新 stop() + 等待 80ms 确保引擎状态干净
- **合成阻塞处理增强**
  - 通过 synthForChunk 状态检查判断合成是否真正完成
  - 在超时情况下取消合成并重新启动，避免无限等待
  - 增加 generation 验证防止过期回调影响当前播放状态
...
- **超时机制优化**
  - mainHandler 超时检测：避免 ttsHandler 线程阻塞导致的超时检测失效
  - 80ms 延迟清理：确保页面切换时引擎完全重置
  - 合成阻塞处理：增强对静默丢弃或阻塞的检测和恢复能力
...
- 延迟合成问题
  ...
  - **超时机制问题**：确认 mainHandler.postDelayed 能正常执行，避免 ttsHandler 阻塞
  - **页面切换清理问题**：检查 80ms 延迟是否足够确保引擎完全重置
- generation 管理
  - 循环播放：确认 speakGeneration 是否正确递增
  - 并发操作：验证 speakGen 是否能防止过期回调影响当前播放
...
**新增的重要改进包括：**
- **超时机制优化**：将 4 秒超时检测从 ttsHandler 改为 mainHandler，避免线程阻塞导致的超时检测失效
