...
- 更新了handleSpeak、handlePreSpeak、handlePause、handleFocusLossPause等方法中的线程处理逻辑
- 确保所有TTS操作都在ttsHandler线程上执行，消除了竞态条件
- 增强了延迟合成策略和页面切换时的竞争问题处理
...
### 重大线程安全改进
**更新** 基于Applied Changes: 重大线程安全改进，修复了主线程调用tts.stop()导致的静默丢弃问题

- **handleSpeak方法中的线程处理改进**
  - 确保所有TTS操作都在ttsHandler线程上执行，避免主线程调用tts.stop()导致的静默丢弃
  - 通过ttsHandler.postDelayed确保tts.stop()和playChunkOnly在同一线程顺序执行
  - 4秒超时保底机制：防止合成被引擎静默丢弃，使用mainHandler而非ttsHandler执行超时检查
  - 80ms延迟策略：确保引擎完全重置，解决页面切换时的竞争问题

- **handlePreSpeak方法中的线程处理改进**
  - 在ttsHandler线程上调用tts.stop()，确保与合成在同一线程执行
  - 200ms延迟策略：确保上一个页面的handleStop在ttsHandler上投递的tts.stop()完全执行
  - 预合成模式下isStopped=true但允许合成，通过isPreSynthesis标志区分

- **handlePause方法中的线程处理改进**
  - 在ttsHandler线程上调用tts.stop()，避免主线程调用导致的竞态条件
  - speakGen++从顶部移至各分支内部，保持OnCompletionListener的有效性
  - 修复暂停恢复后只能播放一个chunk的问题

- **handleFocusLossPause方法中的线程处理改进**
  - 在ttsHandler线程上调用tts.stop()，确保音频焦点丢失时的安全停止
  - 与handlePause相同的线程处理策略，避免竞态条件

- **线程安全机制总结**
  - ttsHandler：专门用于TTS合成和播放操作的线程
  - mainHandler：用于UI更新和回调通知的线程
  - 所有TTS相关操作（tts.stop()、synthesizeToFile、speak）都在ttsHandler线程执行
  - 避免了主线程调用TTS API导致的静默丢弃问题

**章节来源**
- [TTSForegroundService.java:640-711](file://android/app/src/main/java/com/tehui/offline/TTSForegroundService.java#L640-L711)
- [TTSForegroundService.java:782-803](file://android/app/src/main/java/com/tehui/offline/TTSForegroundService.java#L782-L803)
- [TTSForegroundService.java:847-864](file://android/app/src/main/java/com/tehui/offline/TTSForegroundService.java#L847-L864)
- [TTSForegroundService.java:888-900](file://android/app/src/main/java/com/tehui/offline/TTSForegroundService.java#L888-L900)
- [TTSForegroundService.java:978-989](file://android/app/src/main/java/com/tehui/offline/TTSForegroundService.java#L978-L989)
...
## 结论
本服务管理方案通过前台服务、媒体会话、音频焦点与唤醒锁组合，确保 TTS 在后台稳定运行；通过分片与预生成策略、精确的位置与进度上报，提供流畅的用户体验；通过插件桥接与启动画面联动，实现 JS 与原生层的高效协作。最新的重大线程安全改进修复了主线程调用tts.stop()导致的静默丢弃问题，通过确保所有TTS操作都在ttsHandler线程上执行，消除了竞态条件，显著提升了系统的稳定性和可靠性。详细的日志记录功能为调试和性能监控提供了有力支持。

**新增的重要改进包括：**
- **重大线程安全改进**：修复了主线程调用tts.stop()导致的静默丢弃问题
- **handleSpeak方法优化**：确保所有TTS操作都在ttsHandler线程执行，4秒超时保底机制防止合成被引擎静默丢弃
- **handlePreSpeak方法优化**：在ttsHandler线程上调用tts.stop()，200ms延迟确保引擎完全重置
- **handlePause方法优化**：在ttsHandler线程上调用tts.stop()，修复暂停恢复后只能播放一个chunk的问题
- **handleFocusLossPause方法优化**：在ttsHandler线程上调用tts.stop()，确保音频焦点丢失时的安全停止
