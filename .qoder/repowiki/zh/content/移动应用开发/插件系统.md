...
**更新** 本版本新增了 NativeTTSPlugin 的 warmup() 预热功能，显著改善了 TTS 引擎的启动性能和用户体验。
...
SVC["TTSForegroundService<br/>语音合成/播放<br/>支持预热/预合成"]
SYS["系统安装器/分享器"]
end
MA --> AP
MA --> CL
...
**更新** NativeTTSPlugin 新增了 warmup() 方法，用于预热 TTS 引擎，显著改善首次播放的延迟问题。
...
**更新** 新增了 TTS 引擎预热的性能优化说明，这是本次更新的核心改进。
...
**更新** 本次更新新增的 NativeTTSPlugin warmup() 预热功能显著提升了 TTS 引擎的启动性能，改善了用户的首次播放体验，体现了插件系统在性能优化方面的持续改进。
