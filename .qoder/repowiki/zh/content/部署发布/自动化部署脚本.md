...
## 架构总览
下图展示从脚本到主程序再到产物的整体流程，包括新增的访问时间控制中间件生成：
...
通过遵循本文档，团队可在不同平台高效、稳定地完成构建与发布，同时利用访问时间控制功能实现灵活的服务访问管理。
...
- config.yaml
  ...
  - **access_time.allow_days**：允许访问的星期列表
- 环境变量
  - OBFUSCATE_JS：强制开启/关闭混淆（1/0或true/false）
  - CI/GITHUB_ACTIONS：未显式设置时，CI环境默认开启混淆
- app_config.json
