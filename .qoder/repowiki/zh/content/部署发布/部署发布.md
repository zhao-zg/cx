...
- 构建与发布脚本
  ...
  - release.bat：Windows专用发布脚本，封装构建、打包与签名流程。
- Capacitor配置
  - 顶层capacitor.config.json与Android资源中的同名文件共同决定应用行为、资源路径与原生能力。
- 配置文件
  - app_config.json：应用运行时配置（如服务器地址、功能开关等）。
...
## 结论
通过统一的构建脚本、明确的配置文件与工具链，CX项目实现了Web与Android两端的自动化部署。遵循本文档的流程与最佳实践，可高效完成从开发到发布的全链路交付，并在CI/CD中实现持续集成与持续部署。
...
- 版本与更新
  - 使用generate_version.py生成版本号，encrypt_app_update.py加密应用更新包。
- 数据与资源
